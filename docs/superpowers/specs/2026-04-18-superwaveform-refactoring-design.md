# MODUS & SuperWaveform 架构重构与功能设计方案

**日期**: 2026-04-18
**主题**: 框架解耦、协议抽象与上位机体验升级

## 1. 背景与目标
随着 MODUS 框架在项目中的深入应用，原本混杂在 `utilities` 中的调试工具（如 mwaveform, mshell 等）与纯粹的数据结构发生了职责耦合；同时，SuperWaveform 上位机在源码层面仍依赖 MODUS 目录，且缺乏离线数据分析及灵活交互的能力。
本次设计目标：实现纯粹的组件解耦、协议的抽象可替换化，并为上位机注入录制、导出及过滤等专业级功能。

## 2. 架构设计：MODUS 目录重构
现有 `utilities` 目录将严格收敛为“**通用数据结构与算法**”。
新增 `mdebug` 目录作为“**调试与可视化子系统**”。

- **mdebug/**:
  - `mshell.c/h`, `mwaveform.c/h`
  - `trace.c/h`, `trace_fmt.c/h`, `util_debug.c/h`
  - `segger_rtt/`
  - 新增 `mwaveform_protocol.h` (协议接口定义)
- **utilities/**:
  - `mringbuf.c/h`, `util_queue.c/h`, `list.c/h`, `utilities.h`
...
```c
// mdebug/mwaveform_protocol.h
typedef struct {
    uint16_t (*pack_data)(uint8_t *buffer, const int16_t *samples, const uint8_t *mask, uint8_t count);
    uint16_t (*pack_desc)(uint8_t *buffer, const mwaveform_ch_desc_t *channels, uint8_t count);
    // ...
...
### 3.2 面向对象化控制 (mwaveform)
移除 `INIT_SECTION` 的隐式自动注册，采用显式接口调用。
对外暴露统一的 API 对象：
```c
typedef struct {
    int  (*Init)(const mwaveform_protocol_t *protocol);
    void (*Step)(void);
...
```
**弱函数调度机制**：在 `modus_Clock()` (1ms) 中调用弱函数 `mwaveform_Default_Step_Callback()`。默认该回调执行 `mwaveform.Step()`。若用户需在例如 10kHz PWM 中断中调度，只需重写该回调为空，并在中断中调用 `mwaveform.Step()`。

## 4. 上位机 (SuperWaveform) 功能设计

### 4.1 源码层级隔离
在 `superwaveform/src/` 下新建 `utils/` 目录，将 `mringbuf.c/h` 完整复制过去，彻底删除 Makefile 中对 `../../modus/utilities` 的依赖。
