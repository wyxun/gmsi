# MODUS 日志与 Trace 系统架构指南

本文档面向开发者，深入解析 MODUS 框架中 `MLOG` 日志系统及其底层 `Trace` 模块的实现机制。

---

## 1. 核心架构

MODUS 的日志系统采用分层设计，从上到下分为：应用接口层、过滤控制层、多态转换层、以及硬件适配层。

```mermaid
graph TD
    A[应用代码: MLOG/MLOGF] --> B[编译期过滤: MODUS_LOG_LEVEL]
...
为了兼顾性能与灵活性，`MLOG` 采用了两级过滤：

### 2.1 编译期裁剪
通过 `MODUS_LOG_LEVEL` 宏控制。如果编译级别设为 `INFO`，那么所有 `DEBUG` 级别的日志代码在预处理阶段就会被剔除，完全不占据 Flash 空间，也无运行开销。

### 2.2 运行期开关
通过 `g_chGLogMask` 位掩码控制。
- 每个级别对应一个 bit（E=bit0, W=bit1, I=bit2, D=bit3）。
- 开发者可以通过 `mshell` 的 `log` 命令动态修改该掩码，在不重新烧录的情况下开关特定级别的输出。

---

## 3. Trace 模块：C11 多态实现

`Trace` 模块是 MODUS 处理复杂类型转换的核心。它利用 C11 的 `_Generic` 关键字实现了类似 C++ 函数重载的效果。
...
### 5.2 业务模块 .Init() —— 功能必须
与 `TRACE.Init` 不同，业务模块（如 `mwaveform.Init`）是**必须调用**的。
...
> [!TIP]
> **性能建议**：虽然 `MLOG` 很方便，但在极高频（如 10kHz 以上）的中断内，建议优先使用 `mwaveform` 进行数据监测，因为日志的字符串格式化开销仍远大于二进制波形传输。
