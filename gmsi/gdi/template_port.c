 ...
 *
 * @usage
 *   1. 复制此文件到项目 port 目录，重命名为 port_gdi.c
 *   2. 根据目标芯片，include 对应的 CMSIS/HAL 头文件
 *   3. 在各桩函数中填入实际的硬件操作代码
 *   4. 按需修改 gdi_hardware_t HW 结构体的成员
 */
 ...
 * GPIO 适配
 *
 * 示例：将 gdi_gpio_Set() 对接到 HAL_GPIO_WritePin()
 *
 *   static int32_t led_Set(void *pPriv, gdi_gpio_level_t eLevel) {
...
static int32_t stub_gpio_Set(void *pPriv, gdi_gpio_level_t eLevel)
{
    (void)pPriv;
    (void)eLevel;
    /* TODO: 填入 GPIO 写操作, 例如:
     * GPIOA->BSRR = (eLevel == GDI_GPIO_HIGH) ? GPIO_PIN : (GPIO_PIN << 16);
     */
    return 0;
}
...
static int32_t stub_gpio_Get(void *pPriv)
{
    (void)pPriv;
    /* TODO: 填入 GPIO 读操作, 例如:
     * return (GPIOA->IDR & GPIO_PIN) ? GDI_GPIO_HIGH : GDI_GPIO_LOW;
     */
    return GDI_GPIO_LOW;
}
 ...
 * PWM 适配
 *
 * 示例：将 gdi_pwm_SetDuty() 对接到定时器比较值寄存器
 *
 ...
 * Stream (UART) 适配
 *
 * 示例：将 gdi_stream_Write() 对接到 UART 发送
 *
 ...
 * ADC 适配
 *
 * 示例：将 gdi_adc_Read() 对接到 ADC 单次转换
 *
 ...
 * 外设实例（静态分配）
 *
 * 在此定义各外设的 gdi_xxx_t 实例，并填入对应的函数指针。
...
static gdi_stream_t s_tUartComm = {
    .pPriv    = NULL,
    .fnWrite  = stub_stream_Write,
    .fnRead   = stub_stream_Read,
    .fnIsBusy = stub_stream_IsBusy,
...
static gdi_pwm_t s_tMotorPwm = {
    .pPriv     = NULL,
    .fnSetDuty = stub_pwm_SetDuty,
    .fnEnable  = stub_pwm_Enable,
};

static gdi_adc_t s_tBatAdc = {
    .pPriv  = NULL,
