...
static const gshell_io_t s_tRttIO = {
    .pfcnRead  = s_rtt_read,
    .pfcnWrite = s_rtt_write,
};
#endif /* GSHELL_USE_RTT_DEFAULT */

/*============================ PRIVATE DATA ==================================*/

typedef struct {
    char    achLine[GSHELL_LINE_SIZE]; /* 行缓冲区                       */
    uint8_t chLen;                     /* 当前已缓存字节数                 */
} gshell_cb_t;

static const gshell_cmd_t *s_aptCmds[GSHELL_MAX_CMDS];
static uint8_t             s_chCmdCount = 0;
static gshell_cb_t         s_tShell;
static const gshell_io_t  *s_ptIO = NULL; /* 当前 I/O 后端               */
...
static void cmd_ver(const char *args)
{
    ...
    GLOGF(I, "GMSI v%d.%d.%d.%d\r\n", 
                GMSIVersion.chPurpose,
                GMSIVersion.chInterface,
                GMSIVersion.chMajor,
                GMSIVersion.chMinor);
}
...
{
    (void)args;
    struct xLIST *ptList = gbase_GetBaseList();
    if (ptList == NULL || ptList->uxNumberOfItems == 0) {
        shell_puts("No objects registered.\r\n");
        return;
    }
    shell_puts("GMSI objects:\r\n");
    for (struct xLIST_ITEM *ptItem = ptList->xListEnd.pxPrevious;
         ptItem != &ptList->xListEnd;
         ptItem = ptItem->pxPrevious) {
        gmsi_base_t *ptBase = (gmsi_base_t *)ptItem->pvOwner;
...
static void cmd_post(const char *args)
{
    ...
    int wRet = gbase_EventPost((uint32_t)wId, (uint32_t)wEvent);
...
{
    shell_puts("Log mask:  ");
    shell_puts((g_chGLogMask & GLOG_MASK_E) ? "[E]" : "( )");
...
static void cmd_log(const char *args)
{
    ...
    while (*p != '\0') {
        if (*p == '-' && *(p + 1) != '\0') {
            ...
            uint8_t chBit = 0;
            switch (*p) {
                case 'E': case 'e': chBit = GLOG_MASK_E; break;
...
static const gshell_cmd_t s_tCmdHelp = {
    "help", cmd_help, "Show all commands"
};
static const gshell_cmd_t s_tCmdVer = {
    "ver",  cmd_ver,  "Show GMSI version"
};
static const gshell_cmd_t s_tCmdList = {
    "list", cmd_list, "List GMSI gbase objects (id / event)"
};
static const gshell_cmd_t s_tCmdPost = {
...
{
    (void)wUnused;
    const gshell_cmd_t *ptCmd = (const gshell_cmd_t *)wAddr;
    if (NULL == ptCmd)                   return GMSI_EINVAL;
    if (s_chCmdCount >= GSHELL_MAX_CMDS) return GMSI_ENOMEM;
...
void gshell_Poll(void)
{
    ...
    for (unsigned i = 0; i < hwRead; i++) {
        ...
        /* 普通可打印字符：入行缓冲 (已关闭回显) */
        if (c >= 0x20 && c < 0x7F) {
            if (s_tShell.chLen < (GSHELL_LINE_SIZE - 1)) {
