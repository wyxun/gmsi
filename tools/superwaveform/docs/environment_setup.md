...
本指南详细介绍如何搭建 SuperWaveform 的开发与编译环境。

## 1. 上位机开发环境 (Windows + MSYS2)

SuperWaveform 使用 MinGW-w64 架构的 Clang 编译器进行构建。
...
### 1.3 编译器路径配置
由于 MSYS2 的 `bin` 目录通常不建议加入系统环境变量以免冲突，我们在 `tools/superwaveform/Makefile` 中使用了绝对路径：
...
SuperWaveform 依赖 `imgui` 和 `implot`。
...
### 2.2 切换 ImGui 分支 (重要)
SuperWaveform 的布局依赖 ImGui 的 **docking** 分支：
```bash
cd tools/superwaveform/thirdparty/imgui
