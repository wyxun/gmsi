 ...
 * Includes a marker element (tListEnd) to eliminate edge cases during insertion/removal.
 */
...
{
    uint32_t wItemValue;                    /*< The value being listed. Used to sort the list. */
    struct mlist_item *ptNext;              /*< Pointer to the next item in the list. */
    struct mlist_item *ptPrevious;          /*< Pointer to the previous item in the list. */
    void *pvOwner;                          /*< Pointer to the object (normally a base or message) that contains the list item. */
    struct mlist *ptContainer;              /*< Pointer to the list in which this list item is placed (if any). */
} mlist_item_t;

/* Definition of a list. */
typedef struct mlist
{
    uint32_t wNumberOfItems;                /*< Number of items currently in the list. */
    mlist_item_t *ptIndex;                  /*< Used to walk through the list. Points to the last item returned by a call to MLIST_GET_OWNER_OF_NEXT_ENTRY. */
    mlist_item_t tListEnd;                  /*< List item that contains the maximum possible item value meaning it is always at the end of the list and is therefore used as a marker. */
} mlist_t;

/* Access macros */
#define MLIST_SET_ITEM_OWNER(ptListItem, ptOwner)    ((ptListItem)->pvOwner = (void *)(ptOwner))
#define MLIST_GET_ITEM_OWNER(ptListItem)             ((ptListItem)->pvOwner)
#define MLIST_SET_ITEM_VALUE(ptListItem, wValue)     ((ptListItem)->wItemValue = (wValue))
...
#define MLIST_IS_EMPTY(ptList)                       (((ptList)->wNumberOfItems == 0) ? true : false)
