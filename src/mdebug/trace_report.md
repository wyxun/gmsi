...
> 日期：2026-03-08  
> 目标：将 blm 示例中独立的 trace 实现合并回 modus 库，通过条件编译实现 PC / MCU 双平台兼容，消除冗余并剥离 MCU 端对 `printf`（stdio）的依赖。
...
| # | 文件 | 操作 | 路径 |
|---|------|------|------|
| 1 | `trace_fmt.h` | **新增** | `src/utilities/trace_fmt.h` |
| 2 | `trace_fmt.c` | **新增** | `src/utilities/trace_fmt.c` |
| 3 | `trace.c` | **重写** | `src/utilities/trace.c` |
...
**改动**：
- `C_SOURCES` 中移除 `core/trace_stub.c`，添加 `$(MODUS_DIR)/utilities/trace.c` 和 `$(MODUS_DIR)/utilities/trace_fmt.c`
- `C_DEFS` 中新增两个宏定义：
