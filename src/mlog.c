...
void gerr_Printf(int wErrorNum, char* pFile, int wLine)
{
    ...
    MLOGF(E, "gerr_Printf: %s; in file: %s; in line: %d\n", pErrStr, pFile, wLine);
}

/**
 * Function: mlog_Printf
 * ----------------------------
 ...
 * None
 */
void mlog_Printf(const char *pString, char* pFile, int wLine)
{
    MLOGF(I, "mlog_Printf: %s; in file: %s, in line: %d\n", pString, pFile, wLine);
}
...
{
    MLOGF(I, "gval_Printf: %s: %d; in file: %s, in line: %d\n", pString, wValue, pFile, wLine);
}
