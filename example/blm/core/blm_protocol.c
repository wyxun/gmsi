...
PERFC_PT_WAIT_UNTIL(
    ...
    /* Check header byte */
    switch (this.achFrame[0]) {
        case YMODEM_SOH:
            /* 128 byte packet - receive rest of frame */
            this.hwFrameIdx = 1;
            
            /* Receive remaining bytes with PT yield */
            do {
            PERFC_PT_WAIT_UNTIL(
                (wRet > 0),
                wRet = blm_port_UartRecv(&this.achFrame[this.hwFrameIdx], 1, 10);
            )
                this.hwFrameIdx++;
            } while (this.hwFrameIdx < YMODEM_FRAME_SIZE);
            break;
    ...
    /* Verify sequence number */
    {
        ...
        if (chSeq != (uint8_t)(~chSeqInv)) {
            this.tResult = PROTO_SEQ_ERROR;
            goto label_exit;
        }
        
        if (chSeq != this.chExpectedSeq) {
            /* Allow resend of previous packet */
            if (chSeq == (uint8_t)(this.chExpectedSeq - 1)) {
                /* Duplicate packet - copy and return OK */
                memcpy(pchData, &this.achFrame[3], YMODEM_DATA_SIZE);
                *phwLen = YMODEM_DATA_SIZE;
    ...
    /* Verify CRC */
    {
        uint16_t hwCrcCalc = blm_protocol_Crc16(&this.achFrame[3], YMODEM_DATA_SIZE);
        uint16_t hwCrcRecv = ((uint16_t)this.achFrame[YMODEM_FRAME_SIZE - 2] << 8) | 
                              this.achFrame[YMODEM_FRAME_SIZE - 1];
        
        if (hwCrcCalc != hwCrcRecv) {
