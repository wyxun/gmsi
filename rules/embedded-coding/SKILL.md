...
写代码时必须为调试（aitrace）预留观测点，否则调试时无数据可看：

1. **状态机可观测**：状态迁移必须可被调试看到。
   - 状态切换时用 mdebug 打印：`state: IDLE -> RUN`。
   - 状态机结构体暴露当前状态字段（如 `sm->cur`），供 aitrace 变量查看/波形抓取。
2. **关键变量进 waveform**：影响行为的关键变量（电流、速度、计数、错误标志）应放入 RTT waveform 通道，调试时 `aitrace_wave_capture` 可直接抓取分析。
3. **错误可追踪**：错误处理路径必须打印错误码（mdebug），HardFault 时能通过 RTT 日志回溯到出错模块。
4. **延时/耗时可测**：性能关键路径用 perf_counter 计时，调试时通过 `perfc_get` 观察。
5. **禁止**：调试信息用裸 printf 打乱时序；关键路径无任何观测点；错误静默吞掉。

## 调试协同（与 aitrace skill 联动）

- 调试权威流程见 `aitrace` skill（`tools/aitrace.exe` + OpenOCD RTT，被动优先、侵入需确认）。
- 调试→代码闭环：
  1. HardFault 定位（aitrace_crash_report / map_resolve）→ 回查代码违反的 MISRA 规则（如 17.7 返回值未检查、9.1 未初始化）。
  2. 波形异常（aitrace_wave_capture）→ 回查状态机超时/时序逻辑（L2 状态机约束）。
  3. 变量异常（aitrace_gdb_print）→ 回查初始化、符号转换（L0 MISRA 10.3/9.1）。
- 代码→调试：新模块必须自带观测点（见可观测性规范），调试时才有数据可用。
- 调试发现的代码问题修复后，回归验证必须重跑该模块的 TDD 测试。
